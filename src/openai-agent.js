...
class OpenAIAgent extends BaseAIAgent {
  ...
  async handleMessageResponse(initialMessage, reviewState) {
    ...
    while (true) {
      ...
      if (message.tool_calls && message.tool_calls.length) {
            ...
            try {
              ...
              switch (toolCall.function.name) {
                case "get_file_content": {
                  ...
                  // Mark that this file was inspected before commenting
                ...
                case "add_review_comment": {
                  ...
                  if (!reviewState.processedFiles.has(file_name)) {
                    toolOutput = `Please fetch file content for '${file_name}' first using get_file_content before posting inline comments.`;
                    break;
                  }
        ...
        try {
          core.info(
            `Sending follow-up request to OpenAI with model: ${this.model}`
          );
  ...
  async doReview(changedFiles) {
    ...
    reviewState.messageHistory.push({
      role: "user",
      content: `Changed files for review (${
        changedFiles.length
      } files): ${JSON.stringify(simpleChangedFiles, null, 2)}`,
    });
    ...
    try {
      ...
      reviewSummary = await this.handleMessageResponse(
        initialMessage,
        reviewState
      );
  ...
  getFileContent(args) {
    ...
    return this.getFileContentWithCache(
      pathToFile,
      startLineNumber,
      endLineNumber
    );
