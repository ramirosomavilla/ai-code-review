...
class AnthropicAgent extends BaseAIAgent {
    ...
    try {
      ...
      for (let retries = 0; retries < maxRetries; retries++) {
        try {
          ...
          const backoff =
            initialBackoff * Math.pow(2, retries) + Math.random() * 1000;
          core.warning(
        ...
        input_schema: {
          ...
          properties: {
            ...
            start_line_number: {
              type: "integer",
              description:
                "The starting line number in the diff hunk where the comment should begin! (start_line_number must be strictly greater than first diff hunk line number)",
            },
            end_line_number: {
              type: "integer",
              description:
                "The ending line number in the diff hunk where the comment should end! (end_line_number must be strictly greater than start_line_number and strictly less than last diff hunk line number)",
            },
    ...
    try {
      ...
      const initialUserMessage = {
        ...
        content: [
          {
            type: "text",
            text: `Here are the changed files in the pull request that need review (${
              changedFiles.length
  ...
  async handleMessageResponse(message, tools, reviewState) {
    ...
    if (reviewState.iterationCount >= reviewState.maxIterations) {
      core.warning(
        `Reached maximum number of iterations (${reviewState.maxIterations}). Breaking potential infinite loop.`
      );
      ...
      toolCalls.map(async (toolCall) => {
        try {
          ...
          if (toolCall.name === "get_file_content") {
            const { path_to_file, start_line_number, end_line_number } =
              toolCall.input;
            ...
            if (reviewState.seenToolCalls.has(fileRequestKey)) {
              ...
              toolResponse = await this.getFileContentWithCache(
                path_to_file,
                start_line_number,
                end_line_number
              );
            ...
            if (!reviewState.processedFiles.has(file_name)) {
              toolResponse = `Please call get_file_content for '${file_name}' before adding review comments. Comments must reference actual file lines.`;
            } else {
              toolResponse = await this.addReviewComment(
                file_name,
                start_line_number,
                end_line_number,
                found_error_description,
