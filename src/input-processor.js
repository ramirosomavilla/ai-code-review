...
class InputProcessor {
  constructor() {
    ...
    this._existingInlineComments = new Set();
  }
  ...
  _readInputs() {
    this._repo = sanitizeString(
      core.getInput("repo", { required: true, trimWhitespace: true })
    );
    this._owner = sanitizeString(
      core.getInput("owner", { required: true, trimWhitespace: true })
    );
    this._pullNumber = sanitizeNumber(
      core.getInput("pr_number", { required: true, trimWhitespace: true }),
      { min: 1 }
    );
    this._githubToken = sanitizeString(
      core.getInput("token", { required: true, trimWhitespace: true })
    );
    this._aiProvider = sanitizeString(
      core.getInput("ai_provider", { required: true, trimWhitespace: true })
    ).toLowerCase();
    this._apiKey = sanitizeString(
      core.getInput(`${this._aiProvider}_api_key`, {
        required: true,
        trimWhitespace: true,
      })
    );
    this._model = sanitizeString(
      core.getInput(`${this._aiProvider}_model`, {
        required: true,
        trimWhitespace: true,
      })
    );
    this._failAction = sanitizeBool(
      core.getInput("fail_action_if_review_failed")
    );

    this._includeExtensions = sanitizeString(
      core.getInput("include_extensions")
    );
    this._excludeExtensions = sanitizeString(
  ...
  async _setupGitHubAPI() {
    ...
    const pullRequestData = await this._githubAPI.getPullRequest(
      this._owner,
      this._repo,
      this._pullNumber
    );
  ...
  async _processChangedFiles() {
    const comments = await this._githubAPI.listPRComments(
      this._owner,
      this._repo,
      this._pullNumber
    );
    ...
    // Build existing inline comments index to deduplicate later
    ...
    for (const c of reviewComments) {
      // Key by file, side, start/end line, and body text
      const filePath = c.path || "";
      const side = (c.side || c.start_side || "RIGHT").toUpperCase();
      // For single line comments, GitHub returns only `line`
      const startLine = c.start_line || c.line || 0;
      const endLine = c.line || c.start_line || 0;
      const body = (c.body || "").trim();
      const key = JSON.stringify({ filePath, side, startLine, endLine, body });
      this._existingInlineComments.add(key);
    }
  }
  ...
  _filterChangedFiles(
    ...
    const shouldReview = (file) => {
      ...
      const inAllowedPath =
        !incPath.length || incPath.some((p) => filePath.startsWith(p));
  ...
  async _loadReviewRules() {
    if (this._reviewRulesFile) {
      core.info(
        `Attempting to load review rules from: ${this._reviewRulesFile}`
      );
      try {
        ...
        core.warning(
          `Could not load review rules from ${this._reviewRulesFile}: ${error.message}`
        );
  ...
  _setupReviewTools() {
    ...
    this._fileCommentator = async (
      ...
      // Deduplicate: skip if identical comment already exists
      const key = JSON.stringify({
        filePath,
        side: (side || "RIGHT").toUpperCase(),
        startLine: startLineNumber,
        endLine: endLineNumber,
        body: (comment || "").trim(),
      });

      if (this._existingInlineComments.has(key)) {
        core.info(
          `Skipping duplicate inline comment for ${filePath} ${startLineNumber}-${endLineNumber}`
        );
        return;
      }
      ...
      // Record the just-created comment to prevent duplicates within same run
      this._existingInlineComments.add(key);
    };
  }
    ...
    switch (this._aiProvider) {
      case "openai":
        return new OpenAIAgent(
          this._apiKey,
          this._fileContentGetter,
          this._fileCommentator,
          this._model,
          this._reviewRulesContent
        );
      case "anthropic":
